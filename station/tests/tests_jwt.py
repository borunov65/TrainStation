...
class JWTAuthTests(APITestCase):
    ...
    def test_access_protected_train_endpoint_requires_jwt(self):
        """Accessing movies without JWT should return 401"""
        res = self.client.get(TRAIN_URL)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_access_protected_journey_endpoint_requires_jwt(self):
        """Accessing movies without JWT should return 401"""
        res = self.client.get(JOURNEY_URL)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_access_protected_train_endpoint_with_jwt(self):
    ...
    def test_access_protected_journey_endpoint_with_jwt(self):
        """Use obtained JWT access token to access a protected endpoint"""
        payload = {"email": "test@test.com", "password": "testpassword"}
        res = self.client.post(TOKEN_URL, payload)
        access_token = res.data["access"]
